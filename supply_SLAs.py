...
st.markdown('''This application creates interactive visualizations and returns information related to supply chain SLAs. Uploading a sales report document and a supply chain report document is required for the application to process. Once you upload both documents, processing will begin. Select the full screen button in the top right to view the graphs full screen. Select specific sections of the graph to zoom in and reset axis to return to normal graph view. Select legend icons to select/de-select values for a more/less detailed view. Please note that when there is a large number of shipping reference numbers the x-axis may not be able to display all reference numbers. Scroll over the bars or the tic-marks at the top of the graph to confirm the shipping reference number. The graphs are interactive. Have fun with the visualizations and experiment viewing the data in a variety of ways to find the display that works best for you.''')
